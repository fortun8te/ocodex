...
**If doctor still misses it, check the port.** Some installs bind SearXNG on a
non-default port — this dev machine serves it at `http://127.0.0.1:8888` via
`~/bin/searxng`. Point `SEARXNG_URL` there instead:
...
Don't assume a miss at 8080 means SearXNG is broken or required-broken; it may
just be running elsewhere.
...
| `OCODEX_SCOUT_PROVIDER` | unset | e.g. `groq` — scouts skip the OpenRouter daily cap |
| `GROQ_API_KEY` | unset | BYOK; used when provider is `groq` |
...
python3 scripts/ocodex_managed.py triage OUT          # mechanical gate (also runs at end of launch)
python3 scripts/ocodex_managed.py search "query" --workdir DIR --out-dir OUT
# --web for SearXNG URL harvest. Always scout-only; cannot edit.
```
...
prints a filled SUPERVISOR brief, launches a work-stealing pool (one agent
per slot), waits, writes `triage.json`, and can `--watch-status` the slot
board while workers run. A dry pool defers leftover agents instead of
429-storming.
...
$OUT/
  ...
  triage.json              mechanical gate (crashed / evidence-failed / escalate)
```
...
| Path | Role |
|---|---|
| `scripts/ocodex_managed.py` | `doctor` `run` `launch` `status` `wait` `triage` |
| `scripts/ocodex-status` | thin alias → `status` |
| `scripts/launch_batches.py` | pool, live cap, ledger, checkpoints, signals, triage |
...
| `scripts/capacity.py` | live orslot cap, dry-pool defer |
| `scripts/openrouter.py` | optional `/key` + `/generation` probes |
| `scripts/providers.py` | OpenRouter + Groq model fallback |
| `scripts/claims.py` | parse/check `CLAIM \| C\|I \| …` lines |
| `scripts/triage.py` | mechanical post-run gate → `triage.json` |
| `examples/sample-manifest.json` | first-run scout |
| `examples/searxng-compose.yml` | SearXNG via Docker |
| `examples/providers.json` | copy to `~/.ocodex/providers.json` |

## Troubleshooting

**Dead / empty workers (~25% on stream errors).** Harness retries **once**.
If a session id was captured from `codex exec --json`, the retry is
`exec resume <session>` (same conversation). Otherwise it RESUMEs from the
checkpoint file. Then it leaves it. `status` shows `retrying` then `dead`.
