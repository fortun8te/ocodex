...
def checkpoint_clause(out: Path | str, name: str) -> str:
    ...
    return (
        ...
        "   If the last heartbeat is older than 2 minutes, the slot board marks you STALE "
        "and the supervisor treats stale-without-result as likely-dead.\n"
...
def compact_brief(
    ...
    *,
    ...
    parts = [
        checkpoint_clause(checkpoint_out, name),
        "",
...
def collect_status(out: Path, *, now: datetime | None = None) -> list[dict[str, Any]]:
    ...
    for name in names:
        ...
        if result_status == "ok" or ok is True:
            ...
            state = "dead"
        elif hb_age is not None and hb_age > HEARTBEAT_INTERVAL_SEC:
            state = "STALE"
        elif hb_age is None and elapsed is not None and elapsed > HEARTBEAT_INTERVAL_SEC:
            state = "STALE"
...
def format_status_table(rows: list[dict[str, Any]], *, out: Path | str | None = None) -> str:
    ...
    title = f"ocodex slot board  out={out}  heartbeat stale after {HEARTBEAT_INTERVAL_SEC}s"
    return title + "\n" + "\n".join(lines)
